...
def get_config_endpoints() -> tuple[str, str, str, int]:
    ...
    if config_file.exists():
        try:
            with config_file.open("rb") as f:
                ...
                reranker_sec = cfg.get("reranker", {})
                if isinstance(reranker_sec, dict) and "url" in reranker_sec:
                    reranker_url = reranker_sec["url"]
                elif "reranker_url" in cfg.get("llm", {}):
                    reranker_url = cfg["llm"]["reranker_url"]
                elif "reranker_url" in cfg:
    ...
    return base_url, api_key, reranker_url, proxy_port


def wait_for_services(
    base_url: str, api_key: str, reranker_url: str, timeout_seconds: int = 40
) -> bool:
...
def main() -> NoReturn:
    ...
    try:
        base_url, api_key, reranker_url, proxy_port = get_config_endpoints()
        ...
        if start_models and os.path.exists(SERVE_SCRIPT):
            ...
            wait_for_services(base_url, api_key, reranker_url)
